...
## 면접 답
"발급 시스템의 진실의 출처가 두 곳 — *실시간* 재고 정합성은 Redis Lua, *영속·감사*는 PostgreSQL. RDS db.t3.micro Free Tier로 시작, 운영은 Multi-AZ + 7일 backup + Performance Insights. Aurora는 30만 RPS+ 같은 본격 트래픽에서 가치, 캡스톤엔 RDS로 충분."
